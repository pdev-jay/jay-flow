...
대화 ──▶ plan (수정류는 red 증거부터 + 상시 5축 + 조건부 축 체크 + 사용자 수정 반복, 승인까지)
              ...
              ▼
         build (베이스 신선도 확인 → 구현은 전부 builder(sonnet): 공유분 순차 → 독립분 병렬(wide refactor는 expand→migrate→contract) →
...
| `jay-flow:build` | 베이스 신선도 게이트(`git fetch`, 뒤지면 정지), 구현은 전부 `builder`(sonnet) — 공유분 순차·독립분 병렬·wide refactor는 순서 고정, 오라클 green(캡 3, 초과 시 메인 에스컬레이션), 판정은 메인, task green마다 체크포인트 커밋, 통합 red는 귀속→repair 사다리, 전체 green 후 단순화 pass(동작·계약 불변) |
