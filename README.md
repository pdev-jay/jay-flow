...
- **교정은 재개, 재설계는 새 슬롯.** builder의 `agentId`를 기록해 두고, drift(초과·누락·이탈)처럼 담당 파일 안에서 끝나는 기계적 수정은 **같은 builder를 `SendMessage`로 재개**한다 — 완료된 builder도 transcript째 되살아나므로 plan slice·파일 재독이 없고, 메인의 판정 슬롯도 구현 잔해로 안 채운다. 반대로 **캡을 소진한 red task와 트리가 되돌려진 task는 재개하지 않는다** — 전자는 틀린 접근에 그대로 앉히는 것이고, 후자는 builder의 기억과 트리가 어긋난다(에스컬레이션 유지). 재개는 백그라운드로 돌고, `SendMessage`가 없는 환경이면 재-dispatch로 떨어진다 — **최적화이지 절차의 뼈대가 아니다.**
...
대화 ──▶ plan (상시 5축 + 조건부 축 체크 + 사용자 수정 반복, 승인까지)
         ...
         build (베이스 신선도 확인 → 구현은 전부 builder(sonnet): 공유분 순차 → 독립분 병렬 →
                오라클 green → 메인이 drift 판정(교정은 builder 재개),
                막히면 메인 에스컬레이션
...
| `jay-flow:build` | 베이스 신선도 게이트(`git fetch`, 뒤지면 정지), 구현은 전부 `builder`(sonnet) — 공유분 순차·독립분 병렬, 오라클 green(캡 3, 초과 시 메인 에스컬레이션), 판정은 메인(drift 교정은 같은 builder 재개), task green마다 체크포인트 커밋, 통합 red는 귀속→repair 사다리, 전체 green 후 단순화 pass(동작·계약 불변) |
