...
- **구조는 만드는 게 아니라 예측 가능하게 유지하는 것.** 이 루프는 아키텍처를 세우지 않는다 — 기존 경계를 따르고, 어댑터가 하나뿐인 포트를 새로 만들지 않고(그 패턴이 이미 서 있으면 예외 — 혼자 다른 모양이 더 비싸다), 주변 코드처럼 쓴다. **사람 게이트 둘 중 뒤쪽이 사용자가 diff를 직접 읽는 것에 기대기 때문**이다 — 구조가 매번 다르면 읽는 비용이 올라가고 게이트가 형식이 된다. 그래서 강제하는 대신 **이탈을 기록한다**(`구조 관례: 벗어남 — 이유 / 남는 불일치`) — 못 따라가게 만드는 건 이탈이 아니라 기록 안 된 이탈이다. 따를 기존 경계가 없을 때(새 모듈·새 레이어)만 조건부 축이 뜨고, 거기서 묻는 것도 패턴 이름이 아니라 **의존 방향을 무엇이 강제하나**다 — 강제 장치 없는 레이어링은 명명 규칙이고 첫 급한 커밋에서 무너진다.
